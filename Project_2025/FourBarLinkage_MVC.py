...
class FourBarLinkage_Controller():
    ...
    def moveLinkage(self, scenePos):
        ...
        # Update dashpot safely
        try:
            self.FBL_M.DashPot.stPt = self.FBL_M.Pivot1.pt
            self.FBL_M.DashPot.enPt = self.FBL_M.Tracer3.lastPt()
            delta_time = 0.01  # or your simulation time step
            curr_length = self.FBL_M.DashPot.getLength()
            self.FBL_M.DashPot.updateForce(curr_length, delta_time)
        except Exception as e:
            print(f"Dashpot update failed: {e}")
